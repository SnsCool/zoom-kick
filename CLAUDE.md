...
- console.log は `[Bot]`, `[AI]`, `[Sheets]`, `[Notify]` のようにプレフィックスを付ける
- エラーは try-catch で必ず処理し、安全側に倒す（AI判定エラー → パス扱い）
- 日本語コメントOK（このプロジェクトは日本語話者向け）

### ファイル構成のルール

- 1ファイル = 1責務（Bot本体、AI判定、Sheets連携、通知 は全て別ファイル）
...
- Slack Incoming Webhook / LINE Notify (通知)
...
6. **Phase 6**: 通知システム (Slack + LINE)
7. **Phase 7**: Web Dashboard (全UI + Supabase Realtime)
8. **Phase 8**: デプロイ準備 (PM2設定, README)
...
NGワード照合 (即時、0.1秒以下)
  ...
  └── マッチなし → Gemini API に送信 (0.5〜1秒)
       ├── score >= 0.8 → 削除+退出+BAN+記録+通知
       ├── score >= threshold(デフォルト0.7) → 削除+通知 (退出はしない)
